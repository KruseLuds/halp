...
This guide explains how to prepare Home Assistant presence tracking
before installing HALP! (HA Location & Presence analyzer).

HALP! is an analyzer. It does not create a Person, install a phone app,
configure a router, obtain a Bluetooth identity key, or decide which
trackers should be assigned to someone. HALP! works with the location
information Home Assistant already has.
...
Some parts of this guide are optional. A Person can work with only one
tracker. However, multiple independent sources usually give HALP! more
useful evidence to compare.

------------------------------------------------------------------------

# Quick Prerequisite Checklist

Before installing HALP!, try to have the following ready for each
Person:

-   [ ] A Home Assistant Person entity
-   [ ] A phone or other tracked device associated with that Person
-   [ ] A GPS `device_tracker`, usually from the Home Assistant
    Companion App
...
-   [ ] Home Assistant Companion App location permissions configured
    correctly
-   [ ] Battery optimization disabled for the Companion App where
    required
...
A Home Assistant Person is a logical entity that combines one or more
...
-   A router tracker that reports whether the phone is connected to the
    home network

The Person state is influenced by the trackers assigned to it. Home
Assistant also uses tracker source behavior when several trackers
disagree. Review the official Person documentation for the current
rules:
...
Some trackers include latitude, longitude, GPS accuracy, source type, or
other attributes.
...
GPS-capable trackers can place a Person inside Home or another defined
zone.
...
A Home zone that is too small may cause late arrivals or early
departures. A zone that is too large may report Home while the person is
still nearby.
...
-   **GPS:** no HALP! zone assignment. GPS already reports `home`,
    `not_home`, or a named Home Assistant zone.
-   **BLE:** exactly one HALP! zone per BLE tracker entity. A BLE
    identity identifies a device, not the physical receiver location. If
    the same physical BLE device is detected at several sites, Home
    Assistant must expose separate location-specific BLE tracker
    entities before HALP! can assign those entities to different zones.
-   **WiFi/router:** exactly one zone per fixed tracker entity. Use
    clear location-specific entity names when different fixed networks
    or controllers represent different zones.
-   **Mobile WiFi/router:** choose **None / Mobile** in HALP!. Such a
    tracker remains useful for diagnostics but does not vote for an
    absolute geographic zone because its physical network location can
    move.

HALP! stores zone assignments by zone entity ID. When Configure is
opened and saved, deleted trackers and deleted zones are removed from
the saved HALP! mapping, and newly created active zones are available
for selection.

The same reliability threshold, source weights, and freshness rules
apply to every zone. Users do not configure separate weights for Home,
...
5.  First configure and test the GPS, BLE, and router/WiFi sources
    described below.
6.  Return to the Person and assign the trackers after each one is known
    to work.
...
More than one tracker of the same type is possible, but start simply.
Adding several weak trackers can create more confusion rather than more
reliability.
...
For the most dependable results, the phone should normally keep these
features enabled:
...
The phone may also delay reconnecting after arrival because of sleep,
roaming, or battery-saving behavior. A router tracker is therefore
useful supporting evidence, but should not normally be the only presence
source.

## Why Bluetooth should stay on

Private BLE Device, Bermuda, ESPresense, and related systems depend on
Bluetooth advertisements.

When Bluetooth is disabled, BLE presence will stop. Some phones also
reduce advertising under aggressive battery-saving modes.

## Why location services should stay on

The Companion App GPS tracker depends on location permissions and device
location services.

Without current GPS data, Home Assistant may miss zone transitions or
continue displaying an old location.

------------------------------------------------------------------------

# 4. Install and Configure the Home Assistant Companion App

The Home Assistant Companion App is the most common GPS source for
Android and iPhone.
...
The app creates a `device_tracker` and can expose supporting sensors
such as battery state, network status, SSID, BSSID, and connection type.
...
5.  Allow notifications if the household uses Home Assistant mobile
    notifications.
6.  Open the app after installation and confirm it can reach Home
    Assistant both on and away from home WiFi.
7.  In Home Assistant, open the Mobile App integration and identify the
    phone's `device_tracker`.

## Permissions are not identical on every phone

Android vendor menus and iOS wording can change over time. Use the
current Companion App documentation together with the phone's system
settings.
...
For Android, the goal is to let the Companion App receive location
updates in the background without being suspended.
...
The official Companion App documentation states that Android background
location requires the appropriate location and nearby-device
permissions.

## Disable manufacturer-specific sleeping behavior

Some Android manufacturers add extra power-management systems beyond
...
When the phone offers battery choices such as Optimized, Restricted, and
Unrestricted, use the choice that allows reliable background activity.
...
4.  Review network-related sensors such as WiFi connection, SSID, BSSID,
    and connection type if you want them available in Home Assistant.
5.  Wait for each enabled sensor to send at least one update.

Not every optional sensor is required by HALP!. The important tracker
for GPS is the phone's `device_tracker`.
...
6.  Review the Companion App's location troubleshooting page if updates
    are missing.
...
For iPhone, Home Assistant relies on the location behaviors allowed by
iOS.
...
iOS manages background execution itself. There is no exact equivalent to
every Android battery menu.

Do not repeatedly force-close the Home Assistant app. Force-closing an
iOS app can interfere with background behavior until the app is opened
again.

Open Home Assistant periodically and after major phone or app updates to
confirm it remains connected and authorized.
...
Do not confuse the GPS `device_tracker` with supporting sensors such as
geocoded location, connection type, or SSID.

## Alternative GPS sources

HALP! can also analyze other GPS-capable `device_tracker` entities, such
as trackers created by iCloud3 or another location integration.

Classify a tracker as GPS only when it represents geographic location
evidence.

HALP! does not ask for a fixed zone for GPS. Home Assistant already
converts GPS position into `home`, a named zone, or `not_home`. HALP!'s
optional fast-transition feature can use a second matching GPS update to
accelerate any confirmed location transition, for example Home → Work,
Work → `not_home`, or School → Park.

------------------------------------------------------------------------

# 8. BLE Presence Tracking

BLE provides local evidence that a phone or device is near the home or
within Bluetooth coverage.

BLE is valuable because it can confirm presence even when GPS is
delayed.
...
HALP! does not require a specific BLE integration. It only requires a
working BLE `device_tracker`.

For HALP! multi-zone analysis, each BLE tracker entity represents one
fixed receiving zone. The same BLE tracker entity must not be assigned
to several HALP! zones. If one physical phone is detectable at several
physical sites, use separate location-specific Home Assistant tracker
entities when the BLE integration provides them.

## Bluetooth coverage

BLE detection requires one or more Bluetooth receivers that can hear the
phone.
...
One receiver may not cover an entire house. Walls, floors, metal,
electrical equipment, and distance can reduce reception.

Place receivers where they provide useful coverage rather than assuming
one centrally located receiver is enough.

------------------------------------------------------------------------

# 9. Private BLE Device and Obtaining the IRK

Modern phones commonly use resolvable private Bluetooth addresses. Their
visible Bluetooth address changes over time.

Home Assistant can recognize these devices when it knows the Identity
Resolving Key, commonly called the IRK.
...
The official page provides several platform-specific methods. The
section titled:

**On Windows - for any devices that will connect to a computer**

is especially useful for phones that can be paired to a Windows
computer.

This procedure has worked for both Android and iOS devices when followed
carefully, but the order matters.
...
5.  Verify the complete IRK value before entering it into Home
    Assistant.
6.  Keep Bluetooth enabled during testing.

Do not publish an IRK or include it in screenshots. Treat it as
...
6.  Wait for Home Assistant or a Bluetooth proxy to receive
    advertisements.
7.  Confirm the integration creates a device tracker or presence entity
...
Do not assume a BLE tracker is reliable because it detected the phone
once.
...
Router tracking reports whether the phone appears connected to the home
network.
...
Router tracking is local and does not require GPS, but it depends on the
phone maintaining a recognizable network identity.
...
Router integrations differ in update frequency and how quickly they
consider a disconnected client away.

For HALP!, a fixed router/WiFi tracker is assigned to exactly one Home
Assistant zone. If you operate fixed networks at several zones, give the
corresponding Home Assistant tracker entities clear location-specific
names so they are easy to assign correctly. A genuinely mobile
router/WiFi source should use **None / Mobile** in HALP! and will not
vote for an absolute zone.

## A DHCP reservation is strongly recommended

Configure the router or DHCP server so the phone receives a consistent
local IP address.
...
The reservation is configured on the router. It maps the phone's network
hardware address to a chosen IP address.

However, a reservation alone is not enough when the phone changes the
MAC address it presents to the network.

------------------------------------------------------------------------

# 11. MAC Address Randomization

Modern Android and iPhone devices use private or randomized WiFi
addresses to reduce tracking across networks.

This is good for privacy on public networks, but it can break
router-based presence tracking when the identity changes for the home
network.

## Why the IP address alone is not enough

The router normally assigns an IP address based on the client's MAC
address.

A router cannot force a phone to keep using the same identity when the
phone presents a different MAC address.

If the phone changes its MAC address:

-   The DHCP reservation may no longer apply
-   The router may create a new client record
-   Home Assistant may track the old client while the phone uses the new
    one
...
Apple and Android implementations change over time. The goal is not
necessarily to disable privacy everywhere. The goal is to ensure the
phone uses a stable, repeatable identity on the trusted home SSID.
...
A manually entered phone IP may create other networking problems and
still does not solve changing MAC identity.
...
A reliable router tracker should reconnect consistently and should not
create new client identities after routine reconnects.

HALP! can show when router evidence disagrees, but it cannot fix router
firmware, client roaming, DHCP, or phone power management.
...
-   A vehicle tracker unless vehicle location intentionally represents
    the Person
-   A tracker that frequently changes identity

HALP! can classify an assigned tracker as Ignore, but Home Assistant
will still use every tracker assigned to the Person. Remove a tracker
from the Person when Home Assistant itself should not use it.
...
This matters because GPS and non-GPS trackers are not always treated
identically.

HALP! does not replace these rules. HALP! separately analyzes the
configured trackers and creates its own Vetted Location result.

When HALP! Ignore is used:

-   HALP! excludes that tracker
-   Home Assistant Person does not exclude it
-   The default Person state may therefore differ from HALP! Vetted
    Location

For automations where the ignored tracker must not influence the
decision, use HALP!'s Vetted Location sensor rather than relying only on
the default Person entity.
...
HALP! becomes useful here because it exposes agreement, freshness,
conflicts, and its own Vetted Location.
...
Works for zone-based tracking, but HALP! has no local BLE or router
evidence to compare.
...
May work for Home and Away presence, but does not provide meaningful
geographic zone tracking away from home.

## Multiple trackers of one type

HALP! supports multiple GPS, BLE, or router trackers. Add them only when
each source provides distinct and useful evidence.
...
-   Do not publish IRKs
-   Avoid screenshots that expose home addresses, SSIDs, IP addresses,
    or unique device identifiers
-   Keep Home Assistant, HACS, integrations, and phone apps updated

HALP! runs locally in Home Assistant and does not use AI during
...
HALP! will not make the phone or router report more accurately, but it
will make their behavior much easier to understand.
